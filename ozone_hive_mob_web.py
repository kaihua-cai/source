...
# set up hive table
"""
CREATE EXTERNAL TABLE agg_rtb_mob_web_ozone
( device_id int,
...
STORED AS TEXTFILE
LOCATION '/user/kcai/prod/agg_mob_web';    
"""
...
agg_q = """select  i.device_id as device_id,
		...
		i.content_category_id AS content_category_id, 
           ...
           FROM
               (    SELECT auction_id_64, device_id, operating_system, 
                     geo_country, width, height, site_domain, content_category_id
                    FROM log_opt_imps
                    WHERE 1=1
		    AND supply_type < 2
		    AND device_id != 0 			
...
insert_q = """INSERT OVERWRITE TABLE agg_rtb_mob_web_ozone
            PARTITION (dd = '%(dd)s', dh = '%(dh)s') \n""" 
    

def run_mob_web_agg(the_hour):
